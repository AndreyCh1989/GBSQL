...
select u.id as user_id, u.username, sum(t.amount) as unpaid_taxes from taxes t
...
where DATE_ADD(t.issued_datetime, INTERVAL tc.expiration_days DAY) < NOW() and p.id is null
group by u.id
